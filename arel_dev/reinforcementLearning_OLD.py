...
class RL_dqn(object):
    ...
    def __init__(self, chromosomes,id_map,epsilon,return_to_state,hidden_units,learning_rate,discount):
        ...
        self.neural_net = FeedForwardNeuralNet(hidden_units)
        ...
        self.ind = None
        self.current_q = None
        self.optimizer = optim.SGD(self.neural_net.parameters(), lr=learning_rate) # learning rate in optimizer is different than RL learning rate
    ...
    def predict_state(self,current_solution):
        ...
        Erand = random.random()
        current_state = np.array(current_state)
        Q = self.neural_net(Variable(torch.from_numpy(current_state).type(torch.FloatTensor)))
    ...
    def update_state(self,new_solution):
        ...
        Q1 = self.neural_net(Variable(torch.from_numpy(new_state).type(torch.FloatTensor)))
