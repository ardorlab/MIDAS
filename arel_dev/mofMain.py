...
class Optimization_Factory(object):
    ...
    def build_reinforcement_learning(self):
        """
        Assigns all of the parts of the reinforcement learning such as solution and
        generation (epochs).

        Parameters: None

        Written by Gregory Delipei. 9/30/2020
        """
        solution_type_ = self.build_solution()
        population_    = self.build_population()
        generation_    = self.file_settings['optimization']['number_of_generations']
        parameters_   = self.file_settings['optimization']["type"]
        fitness_ = self.build_fitness('genetic_algorithm')
        self.optimization = RL.ReinforcementLearning(solution=solution_type_,
                                             population=population_,
                                             generation=generation_,
    ...
    def build_reproduction(self,number_generations):
        ...
        elif 'fixed_problem' in self.file_settings['optimization']:
            foo = self.file_settings['optimization']['fixed_problem']
            if foo:
