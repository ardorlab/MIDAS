...
def trainmodelrpf():
    ...
    Y = np.load(datapath_new+'outputRFP.npy').astype(np.float32)
    ...
    #     patience=10000,
    # )
    model1.compile(loss='customMSE', ## loss MSE
    	optimizer="adam",
    	lr=5e-4,
    ...
    model1.train(epochs=iter, batch_size=batch_size,
                 model_restore_path=midas_data.__path_to_restore_model__[0],
                #  callbacks=[early_stopping_callback],
                 model_save_path=midas_data.__path_to_save_model__[0])
    temp = midas_data.__path_to_save_model__[0]+"-"+str(iter)+".ckpt"
    midas_data.__path_to_restore_model__[0] =temp
