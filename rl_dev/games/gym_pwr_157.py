...
class Simulate3_Core_157(gym.Env):
    ...
    def __init__(self,core_param):
        ...
        self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=(1,len(self.start)+1), dtype=np.float32)
    ...
    def reset(self):
        ...
        self.counter=0
        self.set_state(self.start)
        rstate=np.append(self.get_mapstate(),self.movemap[self.counter])
        return(rstate.reshape(1,len(self.start)+1))
    ...
    def step(self, action):
        ...
        rstate=np.append(self.get_mapstate(),self.movemap[mid])
        return((rstate.reshape(1,len(self.start)+1), reward, done, info))
