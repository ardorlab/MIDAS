...
class Simulate3_Core_157(gym.Env):
    """
    Class for the Simulate3 core loading pattern solutions 
    adapted for Gym environment
    ...
    def __init__(self,core_param):
        super(Simulate3_Core_157, self).__init__()
        ...
        self.movemap = np.linspace(-1,1,26)
        for i in range(nass):
            self.cmap[invent[i]]=cmap_range[i]
        self.order=core_param['Order']
        self.counter=0
        self.maxcounter = len(self.order)
        state0_file = core_param['Start'] 
    ...
    def step(self, action):
        if self.direction=="Forward":
            gorder = copy.deepcopy(self.order)
            gid = self.counter
        else:
            gorder = copy.deepcopy(self.order)
            gorder.reverse()
            gid = self.counter-len(self.order)
        ...
        if self.counter==2*len(self.start)-1:
            mid=0
            self.evaluate(ldir="run")
            reward = self.core_dict['Results']['Fitness']
            info={'Cycle_Length':self.core_dict['Results']["Cycle_Length"],
                  'Max_Boron':self.core_dict['Results']["Max_Boron"],
                  'Fdh':self.core_dict['Results']["Fdh"],
                  'Fq':self.core_dict['Results']["Fq"],
                  'State': self.get_mapstate()}
            self.direction="Forward"
        else:
            ...
            if self.direction=="Forward":
                ...
                mid=-(self.counter-((len(self.start)-2)))       
            reward=0
            info={}
            
        print(mid)
        self.counter+=1
        done = bool(self.counter==2*len(self.order))
        print(self.counter)
        print(done)
    ...
    def render(self, mode='console'):
        ...
        with open(self.trackfile, 'a') as ofile:
            ...
            ofile.write('\n    obs=' + str(self.get_mapstate()) +  '\n    reward=' + str(self.core_dict['Results']['Fitness']) + "\n")
    ...
    def random_design(self):
        ...
        while brestart == True and niter<maxiter:
            ...
            for iass in assembly_types_group:
                self.core_dict['Inventory'][iass]['In_Design'] = 0
            print(f"New Outer Filling Strategy - {niter}")
            for key, value in self.core_dict['Inventory_Groups'].items():
                total_group=0
                niter_i = 0
                if value['Limit']=='Exact':
                    while total_group != value['Limit_Value'] and niter_i<maxiter:
                        avail_choices = []
                        for iloc in avail_locations_group:
                            for iass in value['Values']:
                                proposed_choice = (iloc, iass)
                                if (total_group + 1 <= value['Limit_Value']) and (self.core_dict['Inventory'][iass]['In_Design'] <= self.core_dict['Inventory'][iass]['Max_Limit']-1):
                        ...
                        if len(avail_choices)==0:
                            ...
                            for iass in assembly_types_group:
                                self.core_dict['Inventory'][iass]['In_Design'] = 0
                            print(f"New Inner Filling Strategy - {niter_i}")
                            continue
                        ...
                        self.core_dict['Inventory'][sass]['In_Design']+= 1
            ...
            for iloc in avail_locations:
                for key, value in self.core_dict['Inventory_Groups'].items(): 
                    if value['Limit']!='Exact':
                        ...
                        for iass in value["Values"]:
                            proposed_choice = (iloc, iass)
                            if (self.core_dict['Inventory'][iass]['In_Design'] <= self.core_dict['Inventory'][iass]['Max_Limit']-1) and (subtotal <=value['Limit_Value']-1):
                                avail_choices.append(proposed_choice)
    ...
    def get_mapstate(self):
        ...
        Written by Gregory Delipei 7/14/2022
        """
        mstate=np.zeros(len(self.core_dict['fuel'].keys()),dtype=np.float32)
        it=0
        for key, value in self.core_dict['fuel'].items():
