...
class PARCS_Interface(Solution):
    """
    Centralized solution class for PARCS v3.3.2 code interface calculations.

    Parameters: None

    Written by Gregory Delipei. 01/08/2022
    Modified by Nicholas Rollins. 09/11/2024
    """
    def __init__(self,inp_obj):
        Solution.__init__(self)
        self.input = inp_obj
        self.input.nrow=17 #!TODO: this shouldn't be hardcoded to allow for other core shapes.
        self.input.ncol=17
        self.input.action_lower=-1
        self.input.action_upper=1
        
        self.core_dict = deepcopy(self.input.core_dict)
        
        ## Store functions according to calculation type
        if self.input.calculation_type == "loading_pattern_parcs332":
            self.solution_funcs = Loading_Pattern_Solution(self.input)
        elif self.input.calculation_type == "loading_patternsimple_parcs332":
            self.solution_funcs = Loading_PatternSimple_Solution(self.input)
        elif self.input.calculation_type == "mcycle_loading_pattern_parcs332":
            self.solution_funcs = MCycle_Loading_Pattern_Solution(self.input)
        elif self.input.calculation_type == "mcycle_grouped_loading_pattern_parcs332":
            self.solution_funcs = MCycle_Grouped_Loading_Pattern_Solution(self.input)
        elif self.input.calculation_type == "mcycle_inventory_loading_pattern_parcs332":
            self.solution_funcs = MCycle_Inventory_Loading_Pattern_Solution(self.input)
    
    def evaluate(self): #!TODO: evaluate functions need to be moved here.
        ## Copy input values to solution_funcs state ##
        self.solution_funcs.input = self.input
        self.solution_funcs.name = self.name
        self.solution_funcs.genome = self.genome
        self.solution_funcs.core_dict = self.core_dict
        self.solution_funcs.parameters = self.parameters
        
        self.parameters, self.additional_parameters = self.solution_funcs.evaluate()
        return
    
    def add_additional_information(self,settings): #!TODO: this has been replaced and should be removed.
        """
    ...
    ## Parse Fuel Assembly types ##
        if 'reflectors' in settings["genome"]["chromosomes"]:
            self.core_dict['Reflectors'] = {}
            for key,value in settings["genome"]["chromosomes"]["reflectors"].items():
                refl_it = {}
        ...
        if 'blanket' in settings["genome"]["chromosomes"]:
            self.core_dict['Blanket'] = {}
            for key,value in settings["genome"]["chromosomes"]["blanket"].items():
        ...
        if 'inventory' in list(settings["genome"].keys()):
            ...
            inventory = {}
            for key,value in settings["genome"]["chromosomes"]["fuel"].items():
                inv_it = {}
...
class PARCS_Geometry_Funcs(): #!TODO: only Loading_Pattern_Solution functions have been added here so far.
    """
    Class of functions necessary for defining the geometry for the PARCS v3.3.2 solutions.
    
    Written by Nicholas Rollins. 09/12/2024
    """
    def generate_core(self):
        ...
        core_map = np.array(core_map).reshape((self.input.nrow,self.input.ncol))
        core_id = []
        for i in range(self.input.nrow-1,-1,-1):
            for j in range(self.input.ncol):
                core_id.append((i-8,j-8))
        core_id=np.array(core_id).reshape((self.input.nrow,self.input.ncol,2))
        if self.input.symmetry == 'quarter':
            self.symmetry_axes = ((8,8),(8,16),(16,8))
            core, fuel = PARCS_Geometry_Funcs.quarter_core(self,core_map,core_id)
            if self.input.calculation_type in ["mcycle_loading_pattern_parcs332",
                                                "mcycle_grouped_loading_pattern_parcs332"]:
                self.core_dict['core'] = {
                    'C1': deepcopy(core),
                    'C2':deepcopy(core),
                ...
                self.core_dict['core'] = core
                self.core_dict['fuel'] = fuel
        elif self.input.symmetry == 'octant':
            self.symmetry_axes = ((8,8),(16,16),(16,8))
            core, fuel = PARCS_Geometry_Funcs.octant_core(self,core_map,core_id)
            if self.input.calculation_type in ["mcycle_loading_pattern_parcs332",
                                                "mcycle_grouped_loading_pattern_parcs332"]:
                self.core_dict['core'] = {
                    'C1': deepcopy(core),
                ...
                self.core_dict['core'] = core
                self.core_dict['fuel'] = fuel
        else: #!TODO: input validation should be handled by input_parser
    ...
    def get_full_core(self):
        ...
        Updated by Nicholas Rollins. 09/17/2024
        """
        if self.input.calculation_type in ["loading_pattern_parcs332"]:
            full_core  = {}
        ...
        elif self.input.calculation_type in ["mcycle_loading_pattern_parcs332"]:
            full_core  = {}
            full_core['C1']  = {}
            full_core['C2']  = {}
            full_core['C3']  = {}
    ...
    def octant_core(self, core_map, core_id):
        ...
        for irow in row_iter:
            for icol in col_iter:
                ...
                core_dict[core_map[irow,icol]] = dict_value
                fuel_dict = PARCS_Geometry_Funcs.extract_fuel(core_dict)
        return(core_dict,fuel_dict)

    def extract_fuel(core_dict):
        """
    ...
    def get_quarter_lattice(self):
            ...
            Written by Gregory Delipei 7/24/2022
            """
            core_map = self.input.core_dict['core_map']
...
class PARCS_Tools():
    """
    Class of functions used for the PARCS v3.3.2 solutions.
    
    Written by Nicholas Rollins. 09/16/2024
    """
    def compute_fa_number(self): #!not in use?
        """
    ...
    def random_design(self): #!not in use?
        """
        ...
        assembly_types_group = deepcopy(assembly_types)
        avail_locations_group = deepcopy(avail_locations)
        maxiter=10000 # maximum iterations to avoid infinite loop.
        nfuel, nfuel_sym, nrefl, nrefl_sym = PARCS_Tools.compute_fa_number()
        total_fuel = nfuel
        total=0
        ...
        for key, value in self.core_dict['Inventory_Groups'].items():
            ...
            if value['Limit']=='Exact':
                ...
                while total_group != value['Limit_Value'] and niter<maxiter:
                    ...
                    for iloc in avail_locations_group:
                        for iass in value['Values']:
                            ...
                            if (total_group + symmetry_multiplier <= value['Limit_Value'])\
                              and (self.core_dict['Inventory'][iass]['In_Design'] <= \
                              self.core_dict['Inventory'][iass]['Max_Limit']-symmetry_multiplier):
                ...
                avail_locations = deepcopy(avail_locations_group)
                assembly_types=deepcopy(assembly_types_group)
                total+=total_group
                ...
                for key, value in self.core_dict['Inventory_Groups'].items(): 
                    if value['Limit']!='Exact':
                        ...
                        for iass in value["Values"]:
                            proposed_choice = (iloc, iass)
                            symmetry_multiplier = len(self.core_dict['fuel'][iloc]['Symmetric_Assemblies'])+1
                            if (self.core_dict['Inventory'][iass]['In_Design'] <= \
                              self.core_dict['Inventory'][iass]['Max_Limit']-symmetry_multiplier)\
                              and (subtotal <=value['Limit_Value']-symmetry_multiplier):
                                avail_choices.append(proposed_choice)
...
class Loading_Pattern_Solution(Solution):
    """
    Solution class for designing and executing loading patterns using PARCS v3.3.2 code.

    Parameters: None

    Written by Gregory Delipei. 01/08/2022
    Modified by Nicholas Rollins. 09/09/2024
    """
    def __init__(self, inp_obj):
        Solution.__init__(self)
        self.input = inp_obj
    ...
    def generate_core(self): #!TODO: this has been replaced and should be removed.
        """
        Generates the 17x17 core map with consistent identifiers and treatment of symmetry.
    ...
    def get_full_core(self): #!TODO: this has been replaced and should be removed.
        """
        Generates the 17x17 full fuel core from symmetry.
    ...
    def quarter_core(self,core_map,core_id): #!TODO: this has been replaced and should be removed.
        """
        Generates the quarter core symmetry map.
    ...
    def octant_core(self, core_map, core_id): #!TODO: this has been replaced and should be removed.
        """
        Generates the octant core symmetry map.
    ...
    def extract_fuel(self, core_dict): #!TODO: this has been replaced and should be removed.
        """
        Extracts the unique fuel assemblies from the core in a dictionary.
    ...
    def get_quarter_lattice(self): #!TODO: this has been replaced and should be removed.
            """
            Generates the 17x17 quarter core lattice.
    ...
    def get_full_lattice(self): #!TODO: this has been replaced and should be removed.
            """
            Generates the 17x17 quarter core lattice.
    ...
    def random_design(self): #!TODO: this has been replaced and should be removed.
        """
        Generates a random design following the constraints in the inventory.
                ...
                for key, value in self.core_dict['Inventory_Groups'].items(): 
                    if value['Limit']!='Exact':
                        ...
                        for iass in value["Values"]:
                            proposed_choice = (iloc, iass)
                            symmetry_multiplier = len(self.core_dict['fuel'][iloc]['Symmetric_Assemblies'])+1
                            if (self.core_dict['Inventory'][iass]['In_Design'] <= self.core_dict['Inventory'][iass]['Max_Limit']-symmetry_multiplier)\
                                and (subtotal <=value['Limit_Value']-symmetry_multiplier):
                                avail_choices.append(proposed_choice)
    ...
    def get_mapstate(self): #! where does self.cmap come from?
        """
    ...
    def plot_design(self,filepath):
        ...
        value_map = deepcopy(self.core_dict['core_map'])
    ...
    def compute_fa_number(self): #!TODO: this has been replaced and should be removed.
        """
        Computes the total number of fuel/reflector assemblies in the core with and without symmetry.
    ...
    def new_generate_initial_fixed(self,chromosome_map,gene_groups):
        ...
        while no_genome_found:
            attempts = 0
            self.my_group = deepcopy(gene_groups)
            self.genome = [None]*chromosome_length
    ...
    def get_lcoe(self): #!TODO: make enrichment a .yaml input parameter
        
        cycle_param={'EFPD': self.parameters['cycle_length']['value'],
                    'Batches': 3,
                    'Thermal_Power': self.input.power,
                    'Efficiency': 0.33,
                    'Fuel_Assemblies': self.input.number_assemblies}
    ...
    def get_results(self,filepath,pin_power=False):
        #!Does pin_power here refer to the pin power reconstruction?
        ...
        if self.parameters["max_boron"]['value'] == 1800.0: #!why is this value special? Is this the BOC guess?
            max_boron =0
    ...
    def evaluate(self): #!TODO: expand to coremaps other than 17x17
            """
            ...
            cdir = self.input.library
            
            if self.input.map_size == 'quarter':
                if self.input.number_assemblies == 193:
                    shutil.copyfile('/home/nkrollin/midas/MIDAS/xslib/' + 'boc_exp_quart193_18.dep', \
                                    self.name +"/" + 'boc_exp.dep') #!TODO:fix this path. what is this file needed for?
            else: #assume full geometry if not quarter-core
                if self.input.number_assemblies == 193:
                    shutil.copyfile('/home/nkrollin/midas/MIDAS/xslib/' + 'boc_exp_full193.dep', \
                                    self.name +"/" + 'boc_exp.dep')
                elif self.input.number_assemblies == 157:
                    shutil.copyfile('/home/nkrollin/midas/MIDAS/xslib/' + 'boc_exp_full157.dep', \
                                    self.name +"/" + 'boc_exp.dep')
            os.chdir(self.name)
            ...
            self.full_core = PARCS_Geometry_Funcs.get_full_core(self)
            if self.input.map_size == 'quarter':
                self.core_lattice = PARCS_Geometry_Funcs.get_quarter_lattice(self)
            else:
                self.core_lattice = PARCS_Geometry_Funcs.get_full_lattice(self)

            xs_array = np.zeros((self.core_lattice.shape[0],self.core_lattice.shape[1]), dtype='<U20')
            pincal_loc = np.zeros((self.core_lattice.shape[0],self.core_lattice.shape[1]), dtype='<U20')
            for x in range(self.core_lattice.shape[0]):
                for y in range(self.core_lattice.shape[1]):
                    loc = self.core_lattice[x,y]
                    if loc != "00" and loc[0] != "R":
                        ...
                        self.core_lattice[x,y] = self.core_dict['Reflectors']['Radial']['Tag']
                        xs_array[x,y] = None
                        pincal_loc[x,y]=0
                    elif loc == "00":
                        self.core_lattice[x,y] = "00" #!TODO: this needs to be justified to match the max Tag length.
                        xs_array[x,y] = None
                        if self.input.map_size == 'quarter':
            ...
            xs_unique = np.unique(xs_array) #list fuel XS files
            xs_unique = np.delete(xs_unique, np.argwhere(xs_unique == 'None'))

            tag_unique = deepcopy(xs_unique)
            if not self.core_dict['Blanket']:
            ...
            #Reflector XS: [Tag, Serial]
            xs_rad = [self.core_dict['Reflectors']['Radial']['Cross_Section'],\
                      self.core_dict['Reflectors']['Radial']['Tag']]
            xs_top = [self.core_dict['Reflectors']['Top']['Cross_Section']]
            xs_bot = [self.core_dict['Reflectors']['Bottom']['Cross_Section']]
            
            fname = 'MIDAS_PARCS_solution' #!TODO: should this be read from the .yaml?
            filename = fname + '.inp'
    ...
    ## CNTL Block Inputs ##
            with open(filename,"a") as ofile:             
                ...
                if not self.input.pin_power_recon:
    ...
    ## GEOM Block Inputs ##
            with open(filename,"a") as ofile:             
                ofile.write("GEOM\n")
                if self.input.map_size == 'quarter':
                    ofile.write("     GEO_DIM 9 9 {} 1 1\n".format(self.input.number_axial))
                else: #assume full geometry if not quarter-core
                    ofile.write("     GEO_DIM 17 17 {} 1 1\n".format(self.input.number_axial))
                ...
                ofile.write("     GRID_Z      {}\n".format('  '.join(self.input.axial_nodes)))
                # Write radial reflector
                ofile.write("     ASSY_TYPE   {}   1*2   {}*2    1*2 REFL\n".format(xs_rad[1],self.input.number_axial-2))
                for i in range(xs_unique.shape[0]):
                    if not self.core_dict['Blanket']:
                        ofile.write("     ASSY_TYPE   {}   1*1 {}*{}  1*3 FUEL\n".format(tag_unique[i],\
                                                                             self.input.number_axial-2,xs_ref[i]))
                    else:
                        ofile.write("     ASSY_TYPE   {}   1*1 1*4  {}*{}  1*4  1*3 FUEL\n".format(tag_unique[i],\
                                                                                       self.input.number_axial-4,xs_ref[i]))
                ofile.write("\n")
                    ...
                    for y in range(pincal_loc.shape[1]):
                        ...
                        try:
                            if np.isnan(val):
                                ...
                                ofile.write(str(pincal_loc[x,y]))
                                ofile.write("  ")
                        except TypeError:
                            ofile.write(str(pincal_loc[x,y]))
                            ofile.write("  ")
    ...
    ## FDBK Block Inputs ##
            with open(filename,"a") as ofile:             
                ofile.write("FDBK\n")
                ofile.write("     FA_POWPIT     {} 21.5\n".format(np.round(self.input.power/self.input.number_assemblies,4)))
    ...
    ## TH Block Inputs ##
            with open(filename,"a") as ofile:   
                ofile.write("TH\n")
                if not self.input.pin_power_recon:
                    ofile.write("UNIF_TH   0.740    626.85     {}\n".format(np.round(self.input.inlet_temperature-273.15,2))) #!TODO: how to deal with av. fuel temp
                else:
                    ofile.write("     FLU_TYP       0\n")
                    ofile.write("     N_PINGT    264 25\n")
                    ofile.write("     PIN_DIM      4.1 4.75 0.58 6.13\n")
                    ofile.write("     FLOW_COND    {}  {}\n".format(np.round(self.input.inlet_temperature-273.15,2),np.round(self.input.flow/self.input.number_assemblies,4)))
    ...
    ## Run PARCS INPUT DECK ##
            ...
            parcscmd = "/cm/shared/codes/TRACE51341_PARCS_332/PARCS-v332_Exe/Executables/Linux/parcs-v332-linux2-intel-x64-release.x" #!TODO: this should be handled by input_parser or a global variable.
    ...
    ## Parse Results from Output
                if 'Finished' in str(output):
                    ofile = fname + '.out'
                    self.get_results(fname,pin_power=False) #!TODO: should pin_power be set by the pin power reconstruction option in PARCS?
                else: #!why would these values be random??
            ...
            return self.parameters, self.additional_parameters


class Loading_PatternSimple_Solution(Solution): #!TODO: modify to use self.input and PARCS_Interface class
    """
...
class MCycle_Loading_Pattern_Solution(Solution): #!TODO: modify to use self.input and PARCS_Interface class
    """
    Solution class for designing loading patterns using PARCS code.
    ...
    def generate_initial(self,chromosome_map):
        ...
        for ci in range(self.ncycles):   
            burnt_fuel_cycle_list = deepcopy(burnt_fuel_list)                                         
            for i in range(chromosome_length):              
                no_gene_found = True                        
                while no_gene_found:
    ...
    def evaluate(self):
            ...
            cdir = self.input.library
            shutil.copyfile('/home/nkrollin/midas/MIDAS/xslib/' + 'mcyc_exp_quarter.dep', 
                            self.name + "/" + 'mcyc_exp.dep')
            os.chdir(self.name)
            ...
            core_lattice_c1 = deepcopy(self.core_lattice)
            ...
            core_lattice_c2 = deepcopy(self.core_lattice)
            ...
            core_lattice_c3 = deepcopy(self.core_lattice)
...
class MCycle_Grouped_Loading_Pattern_Solution(Solution): #!TODO: modify to use self.input and PARCS_Interface class
    """
    ...
    def get_cycle_results(self,filepath,ncyc=1,pin_power=False): #!TODO: this will eventually be the PARCS_Interface shared function.
        efpd=[]
        boron =[]
        fq=[]
        fdh=[]
        ...
        if pin_power:
            zh = np.array([15.24, 10.16, 5.08, 30.48, 30.48, 30.48, 30.48, 30.48,
                        30.48, 30.48, 30.48, 30.48, 30.48, 5.08, 10.16, 15.24]) #!TODO: take axial node values from input_parser.
            asb_mat=self.get_asb_power(filepath)
            pp_mat = self.get_pin_power(filepath)
            fq_asb = np.max(asb_mat)
            fdh_asb = 0
    ...
    def getlp_from_genome(self,cycle_genome,cycle_assb):
        avail_assb = deepcopy(cycle_assb)
        cycle_lp=[]
    ...
    def run_cycle(self,fuel_loc,c0_assb,cycle_lp,exp_file,rdir,ncyc=1):
        ...
        core_cycle_lattice = deepcopy(self.core_lattice)
        ...
        tag_unique_cycle = deepcopy(xs_unique_cycle)
...
class MCycle_Inventory_Loading_Pattern_Solution(Solution): #!TODO: modify to use self.input and PARCS_Interface class
    """
    Solution class for designing multi-cycle loading patterns using PARCS code.
    Each cycle is run as a single-cycle.
    The reloaded fuel assemblies are grouped in pre-defined groups by reactivity.
    ...
    def reproduce_cycle(self):
        old_genome = deepcopy(self.genome)
        nfuel = len(self.core_dict['fuel']['C1'].keys())
        no_gene_found = True
        ncount = 0
        while no_gene_found:  
    ...
    def get_burnup(self,ofile):
        ...
        for i in range(1,len(txt_dep)):
            line_dep = txt_dep[i].split()
            if line_dep[0]=='k':
                asb_counter=deepcopy(iass)
                fasb_counter=deepcopy(ifass)
            elif int(line_dep[0]) not in z_id: 
                pass
            else:
                iz = z_id[-1] - int(line_dep[0])
                ifass = deepcopy(fasb_counter)
                iass = deepcopy(asb_counter)
                for j in range(1,len(line_dep)):
                    iass +=1
                    if iass in refl_id:
                        pass
